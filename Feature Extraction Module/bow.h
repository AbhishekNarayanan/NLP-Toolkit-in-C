...
int first(char *pat, char *txt, int *suffArr, int n,int *pos,int *k, int low,int high) 
{ 
  if(high >= low) 
  { 
    int mid = (low + high)/2; 
...
int last(char *pat, char *txt, int *suffArr, int n,int *pos,int *k,int low,int high) 
{ 
  if (high >= low) 
  { 
  	int m=strlen(pat);
...
void find_occurences(char *pat, char *txt, int *suffArr, int n,int *pos,int *k) 
{ 
  int i; 
  int j; 
...
void displayMinHeap( MinHeap* minHeap,char** vocabulary, int* vocabulary_size ) 
{ 
    int i; 
...
void printKMostFreq( FILE* fp, int k, char** vocabulary, int* vocabulary_size ) 
{ 
...
int** compute_features(int k,const char* train,int isbinary) 
{ 
...
FILE *fp1=fopen(train,"r");
    ...
    while(!feof(fp2))
    {
    	...
    	for(int i=0;i<vocab_size;i++)
    	{
    		if(isbinary==0)
    		{
    			int flag=search(vocabulary[i],buffer,suffArray,len);
 ...
 /*struct bow *features=(struct bow*)malloc(sizeof(struct bow));
    ...
    for(int i=0;i<ndocs;i++)
    {
     ...
     return to_sparse(feature_matrix,ndocs,vocab_size);
}    
